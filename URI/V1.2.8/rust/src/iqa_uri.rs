...
//!   Grammar ......... RFC-009 sec. 10.2 ABNF (sole authority; no syntax added)
//!   Closed sets ..... sec. 10.1 (`organ` and `action` are CLOSED sets -- the
...
//!   Route derivation  SPEC/IQA-URI-ATTEST-v1.2.6 sec. 3 (IQA_ROUTE)
//!
...
/// RFC-009 sec. 10.1 -- action is a CLOSED set (differs from `rttp`).
pub const ACTIONS: [&str; 4] = ["verify", "audit", "attest", "revoke"];

/// RFC-009-C sec. 3 -- standings an Organ may render (closed set, lowercase).
...
pub struct ParsedUri {
    ...
    /// The verb; `None` = omitted (sec. 11.1 standing read).
    pub action: Option<String>,
    pub action_omitted: bool,
    /// RFC-009 sec. 11.3: omitted and `verify` are SAFE; audit/attest/revoke are NOT.
...
///
/// Pure computation, DNS-free (sec. 12 #9); mirrors the RTTP ROUTE_SHARD rule.
...
pub fn parse(uri: &str) -> Result<ParsedUri, IqaUriError> {
    ...
    let rest = uri.strip_prefix(prefix).ok_or_else(|| {
        fail(format!(
            "scheme must be exactly {:?} - no other scheme, no case variant, \
             no fallback (RFC-009 sec. 10.3, sec. 12 #8)",
            prefix
        ))
    })?;
    ...
    for &b in rest.as_bytes() {
        ...
        if "@:?#[]".contains(c) {
            return Err(fail(format!(
                "{:?} is not defined by the scheme (RFC-009 sec. 10.3)",
                c
            )));
        }
        if c.is_ascii_whitespace() {
            return Err(fail(
                "whitespace is not allowed (RFC-009 sec. 10.3 lowercase US-ASCII only)",
            ));
        }
        return Err(fail(format!(
            ...
            if act.is_empty() {
                return Err(fail(
                    "trailing '/' with empty action - action is a closed-set verb, \
                     not omissible by empty string (sec. 10.2)",
                ));
            }
            if act.contains('/') {
                return Err(fail("second path segment - path is a single '/<action>' (sec. 10.2)"));
            }
            (a, Some(act))
        }
        None => (rest, None),
    ...
    if segments.len() != 3 {
        return Err(fail(format!(
            "authority has {} segment(s), must be <subject>.<organ>.<root> (RFC-009 sec. 10.2)",
            segments.len()
        )));
    }
    let (subject, organ, root) = (segments[0], segments[1], segments[2]);
    if subject.is_empty() || root.is_empty() {
        return Err(fail("authority segments must be non-empty (RFC-009 sec. 10.2)"));
    }
    ...
    if !ORGANS.contains(&organ) {
        return Err(fail(format!(
            "organ {:?} is outside the closed set [forge, tss, gateway] (RFC-009 sec. 10.1/sec. 10.3)",
            organ
        )));
    }

    // Action -- CLOSED set (sec. 10.1). The deliberate difference from rttp.
    if let Some(a) = action {
        if !ACTIONS.contains(&a) {
            return Err(fail(format!(
                "action {:?} is outside the closed set [verify, audit, attest, revoke] (RFC-009 sec. 10.1)",
                a
            )));
        }
    }
    ...
    Ok(ParsedUri {
        ...
        action_safe: matches!(action, None | Some("verify")), // sec. 11.3
        canonical_uri,
