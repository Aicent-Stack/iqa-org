...
subject-attestation URI (RFC-009 sec. 10), action safety classes (sec. 11), and the
self-certifying Ed25519 attestation envelope.**

[RFC-009 sec. 10](https://iqa.org/RFC-009/) - sec. 11 - stack v1.2.6
...
...
[PASS] all 75 checks passed (3 skipped)   # core install: the Ed25519 path needs the extra

$ pip install iqa-org[ed25519]
$ python -m iqa.selftest
...
[PASS] all 78 checks passed
```
...
The 78 checks below are the full set, i.e. with the optional Ed25519 backend
installed; a default install runs 75 checks and skips 3.
...
|:---|:---|:---|
| `iqa.iqa_uri` | `iqa://` codec -- validate, canonicalise, classify (8/32/64 hex or name subject), enforce the `organ` and `action` **closed sets**, report `action_safe` | RFC-009 sec. 10.1/sec. 10.2/sec. 10.3 |
...
different acts (RFC-009 sec. 10.4).
...
|:---|:---|:---|
| crates.io | **`iqa-org`** | **published** -- `1.2.8-alpha`. Every published version of the crate is a pre-release, so Cargo will not select it from a plain `iqa-org` requirement: write `cargo add iqa-org@1.2.8-alpha` |
