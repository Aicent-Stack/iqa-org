...
//   Grammar ......... RFC-009 sec. 10.2 ABNF (sole authority)
//   Closed sets ..... sec. 10.1 (organ and action are CLOSED sets)
...
/**
 * RFC-009 sec. 10.1 -- action is a CLOSED set. This is the deliberate difference
...
const SAFE = new Set([null, 'verify']); // sec. 11.3: omitted = standing read; verify = read-only
...
/**
 * IQA_ROUTE = SHA-256(ASCII(canonical_authority))[0:4]  (SPEC sec. 3, decision D1).
 *
...
export function parse(uri) {
  ...
  if (!uri.startsWith(prefix)) {
    throw fail(uri, `scheme must be exactly ${JSON.stringify(prefix)} - no other `
      + 'scheme, no case variant, no fallback (RFC-009 sec. 10.3, sec. 12 #8)');
  }
  const rest = uri.slice(prefix.length);

  // Charset gate: lowercase US-ASCII only (sec. 10.3). One scan rejects uppercase
  // anywhere, userinfo '@', port ':', query '?', fragment '#', whitespace and
  // every other foreign byte.
  for (const ch of rest) {
    if (!ALLOWED.has(ch)) {
      if (FORBIDDEN.has(ch)) {
        throw fail(uri, `${JSON.stringify(ch)} is not defined by the scheme (RFC-009 sec. 10.3)`);
      }
      if (/\s/.test(ch)) {
        throw fail(uri, 'whitespace is not allowed (RFC-009 sec. 10.3 lowercase US-ASCII only)');
      }
  ...
  if (slash !== -1) {
    ...
    if (action === '') {
      throw fail(uri, "trailing '/' with empty action - action is a closed-set "
        + 'verb, not omissible by empty string (sec. 10.2)');
    }
    if (action.includes('/')) {
      throw fail(uri, "second path segment - path is a single '/<action>' (sec. 10.2)");
    }
  }
  ...
  if (segments.length !== 3) {
    throw fail(uri, `authority has ${segments.length} segment(s), must be `
      + '<subject>.<organ>.<root> (RFC-009 sec. 10.2)');
  }
  const [subject, organ, root] = segments;
  if (subject === '' || root === '') {
    throw fail(uri, 'authority segments must be non-empty (RFC-009 sec. 10.2)');
  }
  ...
  if (!ORGANS.includes(organ)) {
    throw fail(uri, `organ ${JSON.stringify(organ)} is outside the closed set `
      + `[${ORGANS.join(', ')}] (RFC-009 sec. 10.1/sec. 10.3)`);
  }
  ...
  return {
    ...
    action_safe: SAFE.has(action),           // sec. 11.3
    canonical_uri: canonicalUri,
    route_hex: deriveRouteHex(authority),    // SPEC sec. 3
  };
}
